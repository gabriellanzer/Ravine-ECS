...
namespace rv
{
    ...
    inline void ComponentsGroup<TComponent>::addComponent(const TComponent* comps, const uint32_t count)
    {
        ...
        memcpy(dataPos() + size, comps + leftCount, rightCount * sizeof(TComponent));
        // Add components before tip
        memcpy(dataPos() + tipOffset - leftCount, comps + 0, leftCount * sizeof(TComponent));
        size += rightCount;
    }
    ...
    inline void ComponentsGroup<Entity>::addComponent(const Entity* comps, const uint32_t count)
    {
        ...
        memcpy(dst, comps + leftCount, rightCount * sizeof(Entity)); // Copy to the end of the group
        for (int32_t i = 0; i < rightCount; i++)                     // Update Entity IDs
        {
            dst[i].id = dst - data + i;
        }
        dst = dataPos() + tipOffset - leftCount;
        memcpy(dst, comps + 0, leftCount * sizeof(Entity)); // Copy components to the left of the tip
    ...
    inline void ComponentsGroup<Entity>::rollClockwise(const int32_t count)
    {
        const int32_t toCopy = min(size, count);
        const int32_t stride = max(size, count);
        Entity* dst = dataPos() + stride;
        memcpy(dst, dataPos(), toCopy * sizeof(Entity)); // Roll data
        for (int32_t i = 0; i < toCopy; i++)             // Update Entity ID
        {
            dst[i].id = dst - data + i;
        }
        tipOffset -= toCopy;                     // Decrease tipOffset
    ...
    inline int32_t ComponentsGroup<Entity>::shiftClockwise(int32_t count)
    {
        ...
        const int32_t rollCount = count * mask;
        Entity* dst = dataPos() + size;
        memcpy(dst, dataPos(), rollCount * sizeof(Entity)); // Roll data
        for (int32_t i = 0; i < rollCount; i++)             // Update Entity IDs
        {
            dst[i].id = dst - data + i;
        }
        dst = dataPos();
        memcpy(dst, dataPos() + rollCount, shiftCount * sizeof(Entity)); // Shift data
        for (int32_t i = 0; i < shiftCount; i++)                         // Update Entity IDs
        {
            dst[i].id = dst - data + i;
        }
        size += count; // Increases size to update end of array
