...
namespace rv
{
	...
	struct CompIt
	{
	  ...
	  public:
		...
		constexpr CompIt(TComp* data, int32_t offset, int32_t size)
		    : data(data), lSize(offset), rSize(size - offset)
		{
		}
		inline ~CompIt() {}
	...
	struct CompGroupIt
	{
		    ...
		    : count(groupCount)
		{
			for (uint8_t i = 0; i < count; i++)
			{
				...
				compIt[i] =
				    CompIt<TComp>(group->data + group->baseOffset, group->tipOffset, group->size);
			}
		}
		~CompGroupIt() { count = -1; }
	};
