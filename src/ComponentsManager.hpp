...
private:
  ...
  static size_t *createRelation() {
    ...
    if (offset == nullptr) {
      ...
      offset = relativityList->linkRelativeOffset(
          getRelativityList<TOtherComponent>());

      // Unless of same type, offset is current array size
      if (static_cast<bool>(
              !std::is_same<TComponent, TOtherComponent>::value)) {
    ...
    if (size == nullptr) {
      ComponentsList *relativityList = getRelativityList<TComponent>();
      size = relativityList->linkRelativeSize(
          getRelativityList<TOtherComponent>());
    }
  ...
  template <class TComponent>
  static void increaseAllOffsets(const size_t startPos,
...
public:
  ...
  static void createComponents(TComponents... args) {
    using expander = int[];
    expander{
        0, ((void)(createComponent<TComponents, TComponents...>(args)), 0)...};
  }
